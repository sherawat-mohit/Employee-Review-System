  ...
  try {
    if (req.isAuthenticated()) {
      ...
      if (alreadyAssigned.length > 0) {
        req.flash('error', `Review is already assigned`);
      ...
      req.flash('error', `Not able to assign review`);
    }
  } catch (err) {
...
module.exports.submitReview = async (req, res) => {
  ...
  try {
    ...
    // preventing from submitting empty feedback
    if (reviewString === '') {
      req.flash('error', `Feedback section cannot be empty`);
      return res.redirect('back');
    }
